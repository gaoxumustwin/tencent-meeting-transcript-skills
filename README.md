...
本仓库的逐字稿导出流程依赖浏览器自动化能力，由 [chrome-devtools-mcp](https://github.com/ChromeDevTools/chrome-devtools-mcp) 提供。使用前需要先安装该 MCP 服务器。

### 前提条件

- [Node.js](https://nodejs.org/) v20.19 或更新的 LTS 版本
- [Chrome](https://www.google.com/chrome/) 当前稳定版或更新版本
- [npm](https://www.npmjs.com/)（Node.js 安装时自带）
- 本地可运行 Python
...
安装后重启 Claude Code，用 `/skills` 确认插件已加载。

> 如果之前已单独安装过 chrome-devtools-mcp，请先移除旧配置再安装插件版本。
...
由于 Windows 需要额外配置 Chrome 路径和启动超时，建议直接编辑 `.codex/config.toml`：
...
安装完成后，在 agent 中输入以下提示确认 MCP 工具可用：

```
检查 https://developers.chrome.com 的性能
```

agent 应自动打开浏览器并执行性能分析，说明 MCP 服务器已正常工作。

更多配置选项和故障排查，参见 [chrome-devtools-mcp 官方文档](https://github.com/ChromeDevTools/chrome-devtools-mcp)。

## 使用前准备

安装好 chrome-devtools-mcp 后，使用前还需确认：
